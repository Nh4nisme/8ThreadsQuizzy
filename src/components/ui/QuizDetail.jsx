import {BookOpen, Calendar, Users, ChartColumnBig, Share2, ChevronLeft, SquarePen} from "lucide-react"
    ...
    <div className="bg-[#151518] border border-gray-800 rounded-md p-6 pb-15">
      ...
      <div className="flex justify-between items-start mb-6">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-3">Students who recently completed this quiz</p>
        </div>
      ...
      <div className="border border-gray-800 rounded-md overflow-hidden">
        <table className="w-full">
          <thead>
            <tr className="border-b border-gray-800">
              {["Student", "Score", "Time Spent", "Completed"].map((h) => (
                <th key={h} className="text-left text-white font-semibold text-sm px-4 py-3">{h}</th>
              ))}
            </tr>
          </thead>
          ...
          <tbody>
            {students.map((s, i) => (
              <tr key={i} className="text-white border-gray-800/50 hover:bg-gray-800/30 transition cursor-pointer">
            ...
            {students.length === 0 && (
              <tr>
                <td colSpan={4} className="text-white text-center px-4 py-8">No completions yet</td>
              </tr>
            )}
          </tbody>
        </table>
...
function QuestionPerformance({ questionsProgress = [] }){
  return(
    <div className="bg-[#151518] border border-gray-800 rounded-md p-6 pb-15">
      ...
      <div className="flex justify-between items-start mb-6">
        <div>
          <h3 className="text-white font-bold text-3xl">Question Performance</h3>
          <p className="text-gray-400 text-sm mt-3">How students performed on each question</p>
        </div>
      </div>
      ...
      <div>
        ...
        <p className="text-gray-500 text-2xl text-center py-8">No data available</p>
        ) : (
          questionsProgress.map((q, i) => (
            <div key={i} className="mb-3">
              <div className="flex justify-between mb-1">
                <span className="text-gray-300 text-sm">{i + 1}. {q.question}</span>
                <span className="text-white font-semibold text-sm">{q.progress}%</span>
              </div>
              <div className="h-1.5 bg-gray-800 rounded-full">
                <div style={{ width: `${q.progress}%` }} className="h-full bg-purple-600 rounded-full" />
              </div>
            </div>
          ))
        )}
    ...
    <div className="flex">
      <div className="flex justify-between w-full items-start bg-[#151518] rounded-md p-6">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-3">Share this quiz with students or colleagues</p>
        </div>
    ...
    <div>
      <div>
       ...
       <div className="grid grid-cols-[60%_40%] gap-4 mt-5">
          ...
          <QuestionPerformance questionsProgress={questionsProgress}></QuestionPerformance>
       </div>
      </div>
    </div>
  )
